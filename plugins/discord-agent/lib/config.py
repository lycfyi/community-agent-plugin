...
class Config:
    ...
    def _load_yaml(self, filename: str) -> dict:
        """Load a YAML config file, auto-creating with defaults if missing."""
        filepath = self._config_dir / filename
        if not filepath.exists():
            # Auto-create config directory and default server.yaml
            if filename == "server.yaml":
                self._create_default_server_config(filepath)
            else:
                return {}
        with open(filepath, "r") as f:
            return yaml.safe_load(f) or {}

    def _create_default_server_config(self, filepath: Path) -> None:
        """Create default server.yaml with placeholder values."""
        filepath.parent.mkdir(parents=True, exist_ok=True)

        config_content = """# Discord Sync Configuration
# Auto-generated - customize as needed

# Storage directory for synced messages
data_dir: "./data"
...
"""
    ...
    def max_messages_per_channel(self) -> int:
        """Get max messages to sync per channel (default 200)."""
        sync_limits = self._server_config.get("sync_limits", {})
        return int(sync_limits.get("max_messages_per_channel", 200))
